...
public:
  // node-ul pentru lista inlantuita de la iterator si initializarea de friend
  // class
  struct Node {
    int value;
    Node *next;
...
public:
  // functiile generice
  Iterator(SpecialContainer::Node *start) { current = start; }

  bool valid() { return current != nullptr; }

  int getCurrent() { return current->value; }
